// 1. parasykite funkcija, kuri priims minutes(number) ir grazins sekundes(string: "x seconds")
...
const arrowMinutesToSeconds3 = (minutes) => `${minutes *60} seconds`;

// console.log(arrowMinutesToSeconds2(2));

// 2. parasyti funkcija, kuri priims vartotoju amziu ir grazins kiek dienu jis nugyveno
//(skaiciu, metu tikslumu, neskaiciuojant keliamuju metu - t.y.visada 365 d.)
...
5.//parasykite funkcija, kuri paims parametra Stringa, ir grazins to parametro paskutine raide
// pvz. paduodu :"Petras", grazina "s"

const lastChar =(string)  => string.charAt(string.lenght -1);
const lastChar1 = (string) =>string[string.lenght -1];
...
const reverseString1 = (s)=>s.spilt("").reverse().join("").toLowerCase();
...
//  console.log(femaleCount(data));
 ...
 const count =femaleCount(data);
//  console.log(`${femaleCount(data)} women`);


 //1. sussikurti arrow funkcija
 //2. isfiltruoti su nurodyta salyga
 //3. grazinti tik id property
 //4.return

 const ids = (array)=>{
     const newerThan2000 = array.filter(person =>person.car_year> 2000);
     console.log(newerThan2000);
...
// sukurkite masyva visu zmoniu, kuriu marskineliu dydziai XS arba S
//IR SURUSIUOKITE SI MASYVA PAGAL VARDUS, A-Z tvarka(alfabetiskai)
...
//pakartokite trecia pratima, kada masyve matytusi tik id, varrdas, marskineliu dydis.
const smallPeopleArray = smallPeople(data);
const uniqueArray = (array) =>array.map((person) => ({
    id: person.id,
